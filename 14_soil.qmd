...
> The workflow worked well at the exception of `r (anti_join(read_tsv("outputs/sites.tsv") %>% select(plot, site) %>% unique(), read_tsv("outputs/soil.tsv") %>% select(plot, site) %>% unique()) %>% select(site) %>% unique())$site %>% paste(collapse = ", ")` . This is because Tene location is wrong and is not in land.
