...
list.files("data/derived_data/soil/", full.names = TRUE) %>%
  read_tsv() %>%
  select(-time) %>%
  gather(variable, value, -site, -plot, -X, -Y) %>%
...
We have information per depth in each plot as shown below, but we will focus here on the top horizon (up to 5cm).
...
data <- read_tsv("data/derived_data/soil.tsv") %>%
  ...
  select(-ocs) %>%
  na.omit()
pca <- prcomp(data %>% select(-site, -plot, -X, -Y, -depth), scale. = TRUE)
autoplot(pca,
...
read_tsv("data/derived_data/soil.tsv") %>%
  filter(depth == 5) %>%
  select(-site, -plot, -X, -Y, -depth, -ocs) %>%
  cor(use = "pairwise.complete.obs") %>%
  corrplot::corrplot(type = "upper", diag = FALSE)
```

We will extract all for modelling but group 2 is focusing on bulk density (bdod) and soil organic carbon (soc) to represent soil fertility.
