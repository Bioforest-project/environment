...
We have information per depth as shown below:
...
This can be plotted as a soil texture graph:

```{r betterplot}
#| message: false
#| warning: false
#| fig-cap: "Soil texture for all available depths."
read_tsv("../data/derived_data/soil.tsv") %>%
  filter(site == params$site) %>%
  ...
  scale_color_viridis_c("Depth [ cm ]")
```
...
read_tsv("../data/derived_data/soil.tsv") %>%
  ...
  arrange(acronym) %>%
  kable(
    caption = "Mean soil data per site for the two upper horizons (up to 15 cm).",
    digits = 2
  )
```
...
All analyses can be found at <https://bioforest-project.github.io/environment/> with corresponding code at <https://github.com/Bioforest-project/environment>. Specifically, the code to generate this document is available at : <https://github.com/Bioforest-project/environment/blob/main/analyses/site_summary.qmd>.
